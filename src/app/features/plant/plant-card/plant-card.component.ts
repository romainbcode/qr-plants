...
@Component({
  ...
  imports: [
    ...
    BadgeStatus,
    PlantCardTemperatureComponent,
    PlantCardExpositionComponent,
    PlantCardHumidityComponent,
    LucideAngularModule,
    CalendarHorizontalComponent,
    MatDialogModule,
  ],
...
export class PlantCardComponent implements OnInit {
  ...
  qrContainer!: ElementRef<HTMLElement>;
    ...
    if (!this.plantService.selectedPlant()) {
      this.plantService
        ...
        .pipe(
          ...
          mergeMap((plant) => {
            this.plantService.setSelectedPlantId(plant.id);
            return this.plantService.getWateringDatesPlantOfMonth(3);
          }),
          tap((dates: any[]) => (this.wateredDates = this.convertToDate(dates)))
        )
        .subscribe();
    }
  }
